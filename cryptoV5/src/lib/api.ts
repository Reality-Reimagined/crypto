...
export const getNews = async () => {
  const url = `https://cryptopanic.com/api/free/v1/posts/?auth_token=${NEWS_API_KEY}&public=true`;
  const response = await axios.get(url);
  return response.data.results;
};
...
export const getDexPairData = async (pairAddress) => {
  const response = await axios.get(`${DEX_SCREENER_BASE_URL}/dex/pairs/${pairAddress}`);
  return response.data;
};

export const searchTokens = async (query) => {
  const response = await axios.get(`${DEX_SCREENER_BASE_URL}/dex/search/?q=${query}`);
  return response.data;
};
